...
/*- Translations -------------------------------------------------------------------- */
include("$root/_admin/_translations/site/$l/users/ts_create_free_account_step_3_image.php");
...
if(isset($_SESSION['user_id'])){
		...
		if($file_type == "jpg" OR $file_type == "jpeg" OR $file_type == "png" OR $file_type == "gif"){
			if(move_uploaded_file($_FILES['inp_image']['tmp_name'], $target_path)) {
				...
				if(is_numeric($width) && is_numeric($height)){
					...
					if($width < 99){
						$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=width_have_to_be_bigger&width=$image_size[0]&height=$image_size[1]";
						header("Location: $url");
						exit;
					}
					if($height < 99){
						$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=height_have_to_be_bigger&width=$image_size[0]&height=$image_size[1]";
						header("Location: $url");
						exit;
					}
					...
					(photo_id, photo_user_id, photo_profile_image, photo_destination, photo_uploaded, photo_uploaded_ip, photo_views, photo_views_ip_block) 
					VALUES 
					(NULL, '$get_my_user_id', '1', $inp_photo_destination_mysql, '$inp_photo_uploaded', $inp_photo_uploaded_ip_mysql, 0, '')")
					...
					$ft = "success";
					$fm = "photo_uploaded";
					$url = "create_free_account_step_3_image.php?action=photo_uploaded&l=$l&ft=$ft&fm=$fm"; 
					header("Location: $url");
					exit;
					...
					$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=file_is_not_an_image";
					header("Location: $url");
					exit;
				}
			}
				...
				switch ($_FILES['inp_image'] ['error']){
					case 1:
						$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=to_big_file";
						header("Location: $url");
						exit;
					case 2:
						$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=to_big_file";
						...
						exit;
					case 3:
						$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=only_parts_uploaded";
						header("Location: $url");
						exit;
					case 4:
						$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=no_file_uploaded";
						header("Location: $url");
						exit;
				}
			...
			$url = "create_free_account_step_3_image.php?l=$l&ft=error&fm=invalid_file_type&file_type=$file_type";
			header("Location: $url");
			exit;
		}
	}
	if($action == ""){
		...
		<p>
		$l_pleased_to_meet_you<br />
		$l_lets_upload_your_image</p>
		...
		<!-- Form -->
			<form method=\"POST\" action=\"create_free_account_step_3_image.php?action=upload&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
		...
		<hr />
		<p>
		<a href=\"view_profile.php?user_id=$my_user_id&amp;l=$l\" class=\"btn btn_default\">$l_skip_this_step</a>
		</p>
		";
	}
	elseif($action == "photo_uploaded"){
		...
		echo"
		<p>
		<a href=\"create_free_account_step_3_image.php?action=rotate&amp;l=$l&amp;process=1\" class=\"btn btn_default\">$l_rotate</a>
		<a href=\"create_free_account_step_3_image.php?action=delete&amp;l=$l\" class=\"btn btn_default\">$l_delete</a>
		<a href=\"create_free_account_step_3_image.php?l=$l\" class=\"btn btn_default\">$l_upload_new_photo</a>
		</p>

		<p>
		<a href=\"view_profile.php?user_id=$my_user_id&amp;l=$l\" class=\"btn btn_success\">$l_continue</a>
		</p>
		";
	}
	elseif($action == "rotate" && $process == "1"){
		...
		if(file_exists("$root/_uploads/users/images/$get_my_user_id/$get_photo_destination")){
			...
			$url = "create_free_account_step_3_image.php?action=photo_uploaded&l=$l";
			header("Location: $url");
			exit;
	...
	elseif($action == "delete"){
		...
		if($get_photo_id != ""){
			...
			if($process == "1"){
				...
				$url = "create_free_account_step_3_image.php?ft=success&fm=photo_deleted&l=$l";
				header("Location: $url");
				exit;
			...
			<p>
			<a href=\"create_free_account_step_3_image.php?action=delete&amp;l=$l&amp;process=1\" class=\"btn btn_warning\">$l_delete</a>
			<a href=\"create_free_account_step_3_image.php?action=photo_uploaded&amp;l=$l\" class=\"btn btn_default\">$l_cancel</a>
			</p>
