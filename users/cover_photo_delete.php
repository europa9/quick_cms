...
if(isset($_SESSION['user_id']) && isset($_SESSION['security']) && $process == "1"){
	...
	if($get_cover_photo_id == ""){
		$url = "cover_photo.php?l=$l&ft=warning&fm=photo_not_found_in_database"; 
		header("Location: $url");
		die;
	}

	if(!(file_exists("$root/_uploads/users/images/$get_user_id/cover_photos/$get_cover_photo_destination"))){
		$url = "cover_photo.php?l=$l&ft=warning&fm=photo_not_found"; 
		header("Location: $url");
		die;
	}
	...
	// Header
	$url = "cover_photo.php?l=$l&ft=success&fm=photo_deleted";
	if($refer != ""){
		$url = $url . "&" . $refer;
	}
