...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$l_recipes - $l_cuisines";
if(file_exists("./favicon.ico")){ $root = "."; }
elseif(file_exists("../favicon.ico")){ $root = ".."; }
elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
...
if($action == ""){
	...
	while($row = mysqli_fetch_row($result)) {
		...
		if($get_cuisine_image != "" && file_exists("$root/$get_cuisine_image")){
			...
			$query_r = "SELECT recipe_id, recipe_user_id, recipe_title, recipe_category_id, recipe_language, recipe_introduction, recipe_image_path, recipe_image FROM $t_recipes WHERE recipe_cusine_id=$get_cuisine_id LIMIT 0,1";
