...
<!-- Newest recipes -->";
	...
	<p class=\"frontpage_category_headline_rightside_p\"><a href=\"$root/recipes/browse_recipes.php?l=$l\" class=\"frontpage_category_headline_rightside_a\" title=\"$l_more_new_recipes\">$l_more</a></p>
	<div class=\"clear\"></div>
	";
	...
	// Get four recipes
	$x = 0;
	$query = "SELECT recipe_id, recipe_title, recipe_category_id, recipe_introduction, recipe_image_path, recipe_image, recipe_thumb_278x156 FROM $t_recipes WHERE recipe_language=$l_mysql ORDER BY recipe_id DESC LIMIT 0,4";
	$result = mysqli_query($link, $query);
	while($row = mysqli_fetch_row($result)) {
...
<!-- Popular recipes -->";
	...
	<p class=\"frontpage_category_headline_rightside_p\"><a href=\"$root/recipes/browse_recipes.php?order_by=recipe_unique_hits&amp;order_method=desc&amp;l=$l\" class=\"frontpage_category_headline_rightside_a\" title=\"$l_more_new_recipes\">$l_more</a></p>
	<div class=\"clear\"></div>
	";
	...
	// Get eight recipes
	$x = 0;
	$query = "SELECT recipe_id, recipe_title, recipe_category_id, recipe_introduction, recipe_image_path, recipe_image, recipe_thumb_278x156 FROM $t_recipes WHERE recipe_language=$l_mysql ORDER BY recipe_unique_hits DESC LIMIT 0,8";
	$result = mysqli_query($link, $query);
	while($row = mysqli_fetch_row($result)) {
		list($get_recipe_id, $get_recipe_title, $get_recipe_category_id, $get_recipe_introduction, $get_recipe_image_path, $get_recipe_image, $get_recipe_thumb_278x156) = $row;
