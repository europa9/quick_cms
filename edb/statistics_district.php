...
/**
*
* File: edb/statistics_district.php 
* Version 1.0
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($get_current_district_id == ""){
		...
		/*- Headers ---------------------------------------------------------------------------------- */
		$website_title = "$l_edb - $get_current_district_title - $l_statistics";
		if(file_exists("./favicon.ico")){ $root = "."; }
		elseif(file_exists("../favicon.ico")){ $root = ".."; }
		elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
		elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
		...
		if($get_my_district_member_id == ""){
			...
			<!-- Months overview -->
					<table class=\"hor-zebra\">
					 ...
					 <tbody>
					";
					$query = "SELECT stats_id, stats_year, stats_month, stats_month_saying, stats_cases_created, stats_cases_closed, stats_avg_created_to_close_days FROM $t_edb_stats_index WHERE stats_district_id=$get_current_district_id ORDER BY stats_id DESC";
					$result = mysqli_query($link, $query);
						...
						echo"
						  <tr>
						   <td"; if($style != ""){ echo" class=\"$style\""; } echo">
							<a href=\"statistics_district_month.php?stats_id=$get_stats_id&amp;l=$l\">$get_stats_month_saying $get_stats_year</a>
						   </td>
						   <td"; if($style != ""){ echo" class=\"$style\""; } echo">
