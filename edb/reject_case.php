			...
			if($process == "1"){
				...
				if($get_current_user_remember_id != ""){
					if($get_current_user_remember_station_id != ""){
						// Go to station
						$url  = "cases_board_2_view_station.php?district_id=$get_current_user_remember_district_id&station_id=$get_current_user_remember_station_id&l=$l&ft=success&fm=case_rejected_($get_current_case_number)";
						header("Location: $url");
						exit;
					}
					else{
						// Go to district
						$url  = "cases_board_1_view_district.php?district_id=$get_current_user_remember_district_id&l=$l&ft=success&fm=case_rejected_($get_current_case_number)";
						header("Location: $url");
						exit;
					}
				} // remember
				// Else go to index
				$url  = "index.php?l=$l&ft=success&fm=case_rejected_($get_current_case_number)";
				header("Location: $url");
				exit;
			}		
		} // access to station
