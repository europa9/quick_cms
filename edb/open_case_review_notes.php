		...
		if($get_my_station_member_id == ""){
			...
			if($action == ""){
				...
				if($process == "1" && ($get_my_station_member_rank == "admin" OR $get_my_station_member_rank == "moderator" OR $get_my_station_member_rank == "editor" OR $get_my_station_member_rank == "editor_limited")){
					...
					$counter = $get_current_menu_counter_review_notes+1;
					$result = mysqli_query($link, "UPDATE $t_edb_case_index_open_case_menu_counters SET menu_counter_review_notes=$counter WHERE menu_counter_case_id=$get_current_case_id") or die(mysqli_error($link));
