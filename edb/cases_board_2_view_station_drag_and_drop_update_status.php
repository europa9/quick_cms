...
if(isset($_POST['case_id']) && isset($_POST['status_id'])){
	...
	if($get_case_id == ""){
		...
		// Find status
		$query = "SELECT status_id, status_parent_id, status_title, status_weight, status_number_of_cases_now, status_on_given_status_do_close_case FROM $t_edb_case_statuses WHERE status_id=$status_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		list($get_new_status_id, $get_new_status_parent_id, $get_new_status_title, $get_new_status_weight, $get_new_status_number_of_cases_now, $get_new_status_on_given_status_do_close_case) = $row;
