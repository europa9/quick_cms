...
/**
*
* File: edb/open_case_evidence_edit_evidence_item_add_password_row.php
* Version 1.0
			...
			if($get_current_item_id == ""){
				...
				if($process == "1"){
					...
					while($row = mysqli_fetch_row($result)) {
						...
						VALUES 
						(NULL, $get_current_case_id, $get_current_item_id, $get_available_id, $inp_available_title_mysql, $get_available_item_type_id, $inp_password_set_new_mysql, NULL, $my_user_id_mysql, $inp_my_user_name_mysql, '$datetime')")
						or die(mysqli_error($link));

						if($focus == "" && $get_available_type != "unlock_pattern"){
							// Get ID, so we can put focus on it
							$query = "SELECT password_id FROM $t_edb_case_index_evidence_items_passwords WHERE password_case_id=$get_current_case_id AND password_item_id=$get_current_item_id AND password_available_id=$get_available_id AND password_item_type_id=$get_available_item_type_id AND password_created_datetime='$datetime'";
							$result = mysqli_query($link, $query);
							$row = mysqli_fetch_row($result);
					...
					$url = "open_case_evidence_edit_evidence_item_item.php?case_id=$get_current_case_id&item_id=$get_current_item_id&mode=item&l=$l&ft=success&fm=password_row_created&focus=$focus#user_and_passwords";
					header("Location: $url");
					exit;
				}
			} // item found
