...
/**
*
* File: edb/open_case_evidence_view_record.php
* Version 1.0
			...
			if($get_current_record_id == ""){
				...
				if($process == "1"){
					...
					while($row_loop = mysqli_fetch_row($result_loop)) {
						...
						if($get_types_item_type_id == ""){

							$result_update = mysqli_query($link, "UPDATE $t_edb_case_index_evidence_items SET 
							item_type_id=NULL,
							item_type_title=NULL
							WHERE item_id=$get_item_id AND item_case_id=$get_current_case_id") or die(mysqli_error($link));
						}
						else{
						...
						if($inp_item_type_id != "$get_item_type_id" && $inp_item_type_id != "0"){
					...
					$url = "open_case_evidence_view_record.php?case_id=$get_current_case_id&record_id=$get_current_record_id&l=$l&ft=success&fm=changes_saved_$save_time";
					header("Location: $url");
					exit;
				}

				echo"
				<h2>$l_record $get_current_record_seized_year/$get_current_record_seized_journal-$get_current_record_seized_district_number</h2>
				...
				<!-- Record actions -->
					...
					if($get_my_station_member_rank == "admin" OR $get_my_station_member_rank == "moderator" OR $get_my_station_member_rank == "editor"){
						echo"
						<p>
						<a href=\"open_case_evidence_new_item.php?case_id=$get_current_case_id&amp;record_id=$get_current_record_id&amp;l=$l\" class=\"btn_default\">$l_add_item</a>
						<a href=\"open_case_evidence_edit_record.php?case_id=$get_current_case_id&amp;record_id=$get_current_record_id&amp;l=$l\" class=\"btn_default\">$l_edit_record</a>
						<a href=\"open_case_evidence_delete_record.php?case_id=$get_current_case_id&amp;record_id=$get_current_record_id&amp;l=$l\" class=\"btn_default\">$l_delete_record</a>
						</p>
						<div style=\"height: 10px;\"></div>
						";
					}
				...
				<!-- List of all items for this record -->
					";
					if($get_my_station_member_rank == "admin" OR $get_my_station_member_rank == "moderator" OR $get_my_station_member_rank == "editor"){
						echo"
						<form method=\"POST\" action=\"open_case_evidence_view_record.php?case_id=$get_current_case_id&amp;record_id=$get_current_record_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
						";
					}
					echo"
				...
				<tr>
				  <td class=\"$style\">
					...
					echo"
					<a href=\"open_case_evidence_edit_evidence_item_info.php?case_id=$get_current_case_id&amp;item_id=$get_item_id&amp;l=$l\">$get_item_record_seized_year/$get_item_record_seized_journal-$get_item_record_seized_district_number-$get_item_numeric_serial_number</a></span>
				  </td>
				  <td class=\"$style\">
					";
					if($get_my_station_member_rank == "admin" OR $get_my_station_member_rank == "moderator" OR $get_my_station_member_rank == "editor"){
						echo"<span><input type=\"text\" name=\"inp_item_title_$get_item_id\" value=\"$get_item_title\" size=\"20\" tabindex=\""; $tabindex = $tabindex+1; echo"$tabindex\" /></span>\n";
					}
					else{
				  ...
				  <td class=\"$style\">
					<span>";
					if($get_my_station_member_rank == "admin" OR $get_my_station_member_rank == "moderator" OR $get_my_station_member_rank == "editor"){
						echo"<input type=\"text\" name=\"inp_item_out_notes_$get_item_id\" value=\"$get_item_out_notes\" size=\"20\" tabindex=\""; $tabindex = $tabindex+1; echo"$tabindex\" />";
					}
					else{
				  ...
				  <td class=\"$style\">
					<a href=\"$root/users/view_profile.php?user_id=$get_item_requester_user_id&amp;l=$l\" title=\"$get_item_requester_user_name\">$get_item_requester_user_first_name $get_item_requester_user_middle_name $get_item_requester_user_last_name</a>
				  </td>
				  <td class=\"$style\">
