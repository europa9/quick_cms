				...
				if($action == ""){
					...
					<!-- Statuses -->
						...
						elseif($get_current_view_method == "person"){
							...
							// Show cases that have status "Tildelt", "Sikres" etc without person assigned to them
							$query_statuses = "SELECT status_id, status_title FROM $t_edb_case_statuses WHERE status_show_on_front_page=1 AND status_on_person_view_show_without_person=0";
							$result_statuses = mysqli_query($link, $query_statuses);
							while($row_statuses = mysqli_fetch_row($result_statuses)) {
								list($get_status_id, $get_status_title) = $row_statuses;

								
								$query_cases = "SELECT case_id, case_number, case_title, case_assigned_to_user_id FROM $t_edb_case_index WHERE case_status_id=$get_status_id AND case_station_id=$get_current_station_id AND case_confirmed_by_human=1 AND case_human_rejected=0 AND case_is_closed=0 AND case_assigned_to_user_id IS NULL";
								$result_cases = mysqli_query($link, $query_cases);
								while($row_cases = mysqli_fetch_row($result_cases)) {
									list($get_case_id, $get_case_number, $get_case_title, $get_case_assigned_to_user_id) = $row_cases;
									echo"<span style=\"color: red;\"><b>Warning</b> <a href=\"open_case_overview.php?case_id=$get_case_id&amp;l=$l\" style=\"color: red;\">$get_case_number $get_case_title</a> missing assigned to<br /></span>\n";
								} // cases
							} // statuses
