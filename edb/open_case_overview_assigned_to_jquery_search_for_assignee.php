...
if(isset($_GET['q']) && $_GET['q'] != ''){
	...
	while($row = mysqli_fetch_row($result)) {
		...
		if($get_station_member_id != "$last_user_id"){
			if($get_station_member_rank == "admin" OR $get_station_member_rank == "moderator" OR $get_station_member_rank == "editor"){
			...
			<!-- Img -->
				...
				if($get_station_member_user_image_file != "" && file_exists("$root/$get_station_member_user_image_path/$get_station_member_user_image_file")){
					if(file_exists("$root/$get_station_member_user_image_path/$get_station_member_user_image_thumb_50") && $get_station_member_user_image_thumb_50 != ""){
						echo"
						<a href=\"#\" class=\"tags_select\" data-divid=\"$get_station_member_user_name\"><img src=\"$root/$get_station_member_user_image_path/$get_station_member_user_image_thumb_50\" alt=\"$get_station_member_user_image_file\" /></a>
						";
					}
					else{
					...
					echo"
					<a href=\"#\" class=\"tags_select\" data-divid=\"$get_station_member_user_name\"><img src=\"_gfx/avatar_blank_50.png\" alt=\"avatar_blank_50.png\" /></a>
					";
				}
		...
		$last_user_id = "$get_station_member_id";
	}
	echo"
		...
		\$(function() {
			\$('.tags_select').click(function() {
				var value = \$(this).data('divid');
            			var input = \$('#autosearch_inp_search_for_assignee');
            			input.val(value);

				// Close
				\$(\".open_case_assignee_reassign\").html(''); 

				// Submit form
				\$(\"#open_case_overview_form\").submit();
            			return false;
       			});
    		});
		</script>
