	...
	if($get_current_case_id == ""){
		...
		if($get_my_station_member_id == ""){
			...
			if($action == ""){
				...
				if($get_current_note_id == ""){


					// Insert
					mysqli_query($link, "INSERT INTO $t_edb_case_index_notes
					(note_id, note_case_id, note_text, note_datetime, note_time, note_date_saying, note_date_ddmmyy, note_user_id, note_user_name, note_user_alias, note_user_email, note_user_image_path, note_user_image_file, note_user_image_thumb_40, note_user_image_thumb_50, note_user_first_name, note_user_middle_name, note_user_last_name) 
					VALUES 
					...
					// Get ID
					$query = "SELECT note_id FROM $t_edb_case_index_notes WHERE note_case_id=$get_current_case_id";
					$result = mysqli_query($link, $query);
					$row = mysqli_fetch_row($result);
					list($get_current_note_id) = $row;
				...
				if($process == "1" && ($get_my_station_member_rank == "admin" OR $get_my_station_member_rank == "moderator" OR $get_my_station_member_rank == "editor")){
					...
					$sql = "UPDATE $t_edb_case_index_notes SET note_text=? WHERE note_id=$get_current_note_id";
					...
					// Update meta data
					$result = mysqli_query($link, "UPDATE $t_edb_case_index_notes SET 
							note_datetime='$inp_datetime', 
							note_time='$inp_time', 
							note_date_saying='$inp_date_saying', 
							note_date_ddmmyy='$inp_date_ddmmyy', 
							note_user_id=$get_my_user_id, 
							note_user_name=$inp_my_user_name_mysql, 
							note_user_alias=$inp_my_user_alias_mysql, 
							note_user_email=$inp_my_user_email_mysql, 
							note_user_image_path=$inp_my_user_image_path_mysql, 
							note_user_image_file=$inp_my_user_image_file_mysql, 
							note_user_image_thumb_40=$inp_my_user_image_thumb_a_mysql, 
							note_user_image_thumb_50=$inp_my_user_image_thumb_b_mysql, 
							note_user_first_name=$inp_my_user_first_name_mysql, 
							note_user_middle_name=$inp_my_user_middle_name_mysql, 
							note_user_last_name=$inp_my_user_last_name_mysql 
							 WHERE note_id=$get_current_note_id") or die(mysqli_error($link));

					// Menu counter
					$query = "SELECT menu_counter_id, menu_counter_notes FROM $t_edb_case_index_open_case_menu_counters WHERE menu_counter_case_id=$get_current_case_id";
					$result = mysqli_query($link, $query);
					$row = mysqli_fetch_row($result);
					list($get_current_menu_counter_id, $get_current_menu_counter_notes) = $row;

					$counter = $get_current_menu_counter_notes+1;
					$result = mysqli_query($link, "UPDATE $t_edb_case_index_open_case_menu_counters SET menu_counter_notes=$counter WHERE menu_counter_case_id=$get_current_case_id");

					$url = "open_case_notes.php?case_id=$get_current_case_id&l=$l&ft=success&fm=changes_saved";
					header("Location: $url");
					exit;
				...
				if($get_my_station_member_rank == "admin" OR $get_my_station_member_rank == "moderator" OR $get_my_station_member_rank == "editor"){
					...
					<!-- //Focus -->
						...
						<!-- Edit page Form -->
						<form method=\"POST\" action=\"open_case_notes.php?case_id=$get_current_case_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
						...
						if($get_current_note_text == ""){
							$get_current_note_text = "<p><b>$inp_date_ddmmyy $get_my_user_name</b><br />-</p>";
						}
						echo"$get_current_note_text</textarea>
						</p>
