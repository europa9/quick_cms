...
/**
*
* File: edb/open_case_evidence_new_item.php
* Version 1.0
* Date 13:47 11.08.2019
...
include("$root/_admin/_translations/site/$l/edb/ts_open_case_evidence_new_record.php");
		...
		if($get_my_station_member_id == ""){
			...
			if($process != "1"){
				...
				<!-- Where am I? -->
					...
					echo"
					&gt;
					<a href=\"open_case_evidence_new_item.php?case_id=$get_current_case_id&amp;record_id=$get_current_record_id&amp;l=$l\">$l_new_item</a>
					</p>
			...
			if($process == "1"){
				...
				$inp_record_id = $_POST['inp_record_id'];
				$inp_record_id = output_html($inp_record_id);
				$inp_record_id_mysql = quote_smart($link, $inp_record_id);
				if($inp_record_id == ""){
					echo"Record id = ";
					die;
				}
				...
				// Get record id
				$query = "SELECT record_id, record_case_id, record_seized_year, record_seized_journal, record_seized_district_id, record_seized_district_number, record_seized_district_title, record_notes, record_created_datetime, record_created_time, record_created_date_saying, record_created_date_ddmmyy FROM $t_edb_case_index_evidence_records WHERE record_id=$inp_record_id_mysql AND record_case_id=$get_current_case_id";
				$result = mysqli_query($link, $query);
				$row = mysqli_fetch_row($result);
				list($get_current_record_id, $get_current_record_case_id, $get_current_record_seized_year, $get_current_record_seized_journal, $get_current_record_seized_district_id, $get_current_record_seized_district_number, $get_current_record_seized_district_title, $get_current_record_notes, $get_current_record_created_datetime, $get_current_record_created_time, $get_current_record_created_date_saying, $get_current_record_created_date_ddmmyy) = $row;
				if($get_current_record_id == ""){
					echo"Record ID not found! (Searched with record id = $inp_record_id)";
					die;
				}

				$inp_seized_year_mysql = quote_smart($link, $get_current_record_seized_year);
				$inp_seized_journal_mysql = quote_smart($link, $get_current_record_seized_journal);
				$inp_seized_district_number_mysql = quote_smart($link, $get_current_record_seized_district_number);
				...
				if(strstr($inp_items, '-')){
					...
					for($x=$start_number;$x<$stop_number;$x++){
						...
						VALUES 
						(NULL, $get_current_case_id, $get_current_record_id, $get_current_record_seized_year, $get_current_record_seized_journal, $get_current_record_seized_district_number, $inp_numeric_serial_number_mysql, 0, 
						...
						$count_number_of_items = $count_number_of_items+1;
					}
				} // series (1-5)
				else{
					// Look for ,
				...
				$url = "open_case_evidence_view_record.php?case_id=$get_current_case_id&record_id=$get_current_record_id&l=$l&ft=success&fm=changes_saved";
				header("Location: $url");
				exit;
			}
			echo"
			...
			<!-- Focus -->
				...
				\$(document).ready(function(){
					\$('[name=\"inp_items\"]').focus();
				});
				</script>
			<!-- //Focus -->

			<!-- New item -->
				<form method=\"POST\" action=\"open_case_evidence_new_item.php?case_id=$get_current_case_id&amp;record_id=$get_current_record_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">


				<p><b>$l_record:</b> (<a href=\"open_case_evidence_new_record.php?case_id=$get_current_case_id&amp;l=$l\">$l_new_journal</a>)<br />
				<select name=\"inp_record_id\" tabindex=\""; $tabindex = $tabindex+1; echo"$tabindex\" />\n";
				$count_records = 0;
				$get_current_record_id = "";
				$query = "SELECT record_id, record_case_id, record_seized_year, record_seized_journal, record_seized_district_number FROM $t_edb_case_index_evidence_records WHERE record_case_id=$get_current_case_id ORDER BY record_seized_year, record_seized_journal, record_seized_district_number ASC";
				$result = mysqli_query($link, $query);
				while($row = mysqli_fetch_row($result)) {
					list($get_record_id, $get_record_case_id, $get_record_seized_year, $get_record_seized_journal, $get_record_seized_district_number) = $row;
					
					if($get_current_record_id == ""){
						$get_current_record_id = "$get_record_id";
					}

					echo"				";
					echo"<option value=\"$get_record_id\""; if($get_record_id == "$get_current_record_id"){ echo" selected=\"selected\""; } echo">$get_record_seized_year/$get_record_seized_journal-$get_record_seized_district_number</option>\n";
				...
				echo"
				<p><b>$l_request_text:</b><br /><span class=\"smal\">
				...
				<textarea name=\"inp_request_text\" rows=\"5\" cols=\"80\" tabindex=\""; $tabindex = $tabindex+1; echo"$tabindex\" style=\"width: 100%;\">";
				$get_prev_item_request_text = str_replace("<br />", "\n", $get_prev_item_request_text); 
				echo"$get_prev_item_request_text</textarea>
				</p>
				...
				<input type=\"text\" name=\"inp_requester_user_name\" id=\"autosearch_inp_search_for_requester\" value=\"$get_prev_item_requester_user_name\" size=\"25\" tabindex=\""; $tabindex = $tabindex+1; echo"$tabindex\" style=\"width: 100%;\" />
				</p>
