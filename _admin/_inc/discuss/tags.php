...
if($action == ""){
	...
	$query = "SELECT tag_id, tag_title, tag_title_clean, tag_introduction, tag_description, tag_is_official, tag_icon_path, tag_icon_file_16 FROM $t_discuss_tags_index ORDER BY tag_title ASC";
...
elseif($action == "open_tag"){
	...
	if($get_current_tag_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "UPDATE $t_discuss_tags_index SET tag_is_official=$inp_is_official_mysql WHERE tag_id=$get_current_tag_id");
					...
					$full_dest  = "../_uploads/discuss/tags_icons/" . $get_current_tag_title_clean . "_16x16." . $extension;
  					...
  					if (move_uploaded_file($tmp_name, $full_dest)){
						...
						if($width == "" OR $height == ""){
							...
							mysqli_query($link, "UPDATE $t_discuss_tags_index SET tag_icon_path=$inp_icon_path_mysql, tag_icon_file_16=$inp_icon_mysql WHERE tag_id=$get_current_tag_id") or die(mysqli_error($link));
					...
					$full_dest  = "../_uploads/discuss/tags_icons/" . $get_current_tag_title_clean . "_32x32." . $extension;
  					...
  					if(move_uploaded_file($tmp_name, $full_dest)){
						...
						if($width == "" OR $height == ""){
							...
							mysqli_query($link, "UPDATE $t_discuss_tags_index SET tag_icon_path=$inp_icon_path_mysql, tag_icon_file_32=$inp_icon_mysql WHERE tag_id=$get_current_tag_id") or die(mysqli_error($link));
					...
					$full_dest  = "../_uploads/discuss/tags_icons/" . $get_current_tag_title_clean . "_256x256." . $extension;
  					...
  					if (move_uploaded_file($tmp_name, $full_dest)){
						...
						if($width == "" OR $height == ""){
							...
							mysqli_query($link, "UPDATE $t_discuss_tags_index SET tag_icon_path=$inp_icon_path_mysql, tag_icon_file_256=$inp_icon_mysql WHERE tag_id=$get_current_tag_id") or die(mysqli_error($link));
			...
			while($row = mysqli_fetch_row($result)) {
				...
				mysqli_query($link, "UPDATE $t_discuss_tags_index_translation SET tag_translation_introduction=$inp_introduction_mysql WHERE tag_id=$get_current_tag_id AND tag_translation_language='$get_language_active_iso_two'") or die(mysqli_error($link));


				$inp_description = $_POST["inp_description_$get_language_active_iso_two"];
				$sql = "UPDATE $t_discuss_tags_index_translation SET tag_translation_description=? WHERE tag_id=$get_current_tag_id AND tag_translation_language='$get_language_active_iso_two'";
			...
			// Header
			$url = "index.php?open=discuss&page=tags&action=open_tag&tag_id=$tag_id&editor_language=$editor_language&ft=success&fm=changes_saved";
		...
		<!-- Form -->
			...
			while($row = mysqli_fetch_row($result)) {
				...
				// Find translation
				$query_t = "SELECT tag_translation_id, tag_id, tag_translation_language, tag_translation_introduction, tag_translation_description FROM $t_discuss_tags_index_translation WHERE tag_id=$get_current_tag_id AND tag_translation_language='$get_language_active_iso_two'";
				...
				echo"
				<hr />
				<p><b><img src=\"_design/gfx/flags/16x16/$flag\" alt=\"$flag\" /> Introduction</b><br />
				...
				<p><b><img src=\"_design/gfx/flags/16x16/$flag\" alt=\"$flag\" /> Description</b><br />
