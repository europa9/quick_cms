...
if($process == "1"){
	...
	while($row = mysqli_fetch_row($result)) {
		...
		$result_update = mysqli_query($link, "UPDATE $t_discuss_titles SET 
					 title_value=$inp_value_mysql
			 WHERE title_language='$get_language_active_iso_two'");

	} // while
	$url = "index.php?open=discuss&page=title&editor_language=$editor_language&ft=success&fm=changes_saved";
	header("Location: $url");
	exit;
} // process
else{
	...
	<!-- Titles -->
		...
		while($row = mysqli_fetch_row($result)) {
			...
			// Get value
			$query_t = "SELECT title_id, title_language, title_value FROM $t_discuss_titles WHERE title_language='$get_language_active_iso_two'";
			...
			if($get_title_id == ""){
				...
				VALUES 
				(NULL, '$get_language_active_iso_two', 'Discuss')")
