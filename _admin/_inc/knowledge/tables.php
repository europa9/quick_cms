...
if($result !== FALSE){
	...
	$query = "SELECT user_id, user_email, user_alias FROM $t_users WHERE user_rank='admin'";
	$result = mysqli_query($link, $query);
	while($row = mysqli_fetch_row($result)) {
		list($get_user_id, $get_user_email, $get_user_alias) = $row;
		...
		VALUES 
		(NULL, 1, 'admin', $get_user_id, $inp_user_alias_mysql, $inp_user_email_mysql, $inp_user_image_mysql, '', '$datetime', '$date_saying', 1, 'Administrator', '')")
		or die(mysqli_error($link));
	}
}
echo"
