...
$result = mysqli_query($link, "DROP TABLE IF EXISTS $t_food_diary_last_used_food") or die(mysqli_error($link)); 


echo"
	<!-- food_diary_last_used_food -->
	";
	$query = "SELECT * FROM $t_food_diary_last_used_food";
	$result = mysqli_query($link, $query);
	if($result !== FALSE){
		// Count rows
		$row_cnt = mysqli_num_rows($result);
		echo"
		<p>$t_food_diary_last_used_food: $row_cnt</p>
		";
	}
	else{
		mysqli_query($link, "CREATE TABLE $t_food_diary_last_used_food(
