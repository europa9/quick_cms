...
if($get_current_food_id == ""){
	...
	if($process == "1"){
		...
		if($inp_food_serving_size_gram != "$get_current_food_serving_size_gram"){
			...
			if($get_current_food_carbohydrates_of_which_dietary_fiber != ""){
				$inp_food_carbohydrates_of_which_dietary_fiber_calculated = round($get_current_food_carbohydrates_of_which_dietary_fiber*$inp_food_serving_size_gram/100, 0);
			}
			else{
				$inp_food_carbohydrates_of_which_dietary_fiber_calculated = $get_current_food_carbohydrates_of_which_dietary_fiber_calculated;
			}
		...
		$url = "index.php?open=$open&page=edit_food_general&main_category_id=$get_current_food_main_category_id&sub_category_id=$get_current_food_sub_category_id&food_id=$get_current_food_id&editor_language=$editor_language&l=$l&ft=success&fm=changes_saved";
		header("Location: $url");
		exit;
	} // process == 1
	...
	<!-- Edit food general -->
		<form method=\"post\" action=\"index.php?open=$open&amp;page=edit_food_general&amp;main_category_id=$get_current_food_main_category_id&amp;sub_category_id=$get_current_food_sub_category_id&amp;food_id=$get_current_food_id&amp;editor_language=$editor_language&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\" id=\"my_form\">
