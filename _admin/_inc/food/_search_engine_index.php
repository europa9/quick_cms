...
if($result_exists !== FALSE){
	...
	while($row_w = mysqli_fetch_row($result_w)) {
		...
		$inp_index_reference_id_mysql = quote_smart($link, "food_id$get_food_id");
	...
	while($row_food = mysqli_fetch_row($result_food)) {
		...
		if($get_index_id == ""){
			// Insert
			echo"<span>Insert category $get_category_translation_value ($get_category_translation_language)<br /></span>\n";
			mysqli_query($link, "INSERT INTO $t_search_engine_index 
			(index_id, index_title, index_url, index_short_description, index_keywords, index_module_name, index_reference_id, index_is_ad, index_created_datetime, index_created_datetime_print, index_language) 
			VALUES 
			(NULL, $inp_index_title_mysql, $inp_index_url_mysql, $inp_index_short_description_mysql, $inp_index_keywords_mysql, $inp_index_module_name_mysql, $inp_index_reference_id_mysql, $inp_index_is_ad_mysql, '$datetime', '$datetime_saying', $inp_index_language_mysql)")
