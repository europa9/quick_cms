...
if($action == ""){
	...
	<!-- List all weeks -->
		...
		$query = "SELECT moderator_of_the_week_id, moderator_week, moderator_year, moderator_user_id, moderator_user_email, moderator_user_name, moderator_user_alias, moderator_comment FROM $t_users_moderator_of_the_week ORDER BY moderator_of_the_week_id ASC";
		$result = mysqli_query($link, $query);
		while($row = mysqli_fetch_row($result)) {
			list($get_moderator_of_the_week_id, $get_moderator_week, $get_moderator_year, $get_moderator_user_id, $get_moderator_user_email, $get_moderator_user_name, $get_moderator_user_alias, $get_moderator_comment) = $row;
	...
	if($last_week_number == ""){
		...
		$weeks_in_year = get_iso_weeks_in_year($current_year);
		for($x=$current_week;$x<$weeks_in_year+1;$x++){
			$year = "$current_year";
			$week = "$x";
