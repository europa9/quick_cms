...
if($get_current_course_id == ""){
	...
	if($action == ""){
		if($process == "1"){
			...
			// Header
			$url = "index.php?open=$open&page=courses_index&category_id=$get_current_category_id&editor_language=$editor_language&ft=success&fm=course_deleted#course$get_current_course_id";
			header("Location: $url");
			exit;
		}

		echo"
		<h1>Delete course $get_current_category_title</h1>
		...
		<!-- Where am I? -->
			...
			&gt;
			<a href=\"index.php?open=courses&amp;page=courses_delete&amp;course_id=$get_current_course_id&amp;editor_language=$editor_language&amp;l=$l\">Delete $get_current_category_title</a>
			</p>
		<!-- //Where am I? -->
