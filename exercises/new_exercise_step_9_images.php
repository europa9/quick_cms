...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($get_exercise_id == ""){
		...
		if($action == "upload_image"){
				...
				if($file_type == "jpg" OR $file_type == "jpeg" OR $file_type == "png" OR $file_type == "gif"){
					...
					if(move_uploaded_file($_FILES['inp_image']['tmp_name'], $target_path)) {
						...
						if(is_numeric($width) && is_numeric($height)){
							...
							if($width < 889){
								unlink($target_path);
								$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$l&ft=error&fm=width_have_to_be_bigger&width=$width&height=$height";
								header("Location: $url");
								exit;
							}
							if($height < 889){
								unlink($target_path);
								$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$l&ft=error&fm=height_have_to_be_bigger&width=$width&height=$height";
								header("Location: $url");
								exit;
							}
							...
							$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$l&ft=error&fm=file_is_not_an_image";
							header("Location: $url");
							exit;
						}
					}
   						...
   						switch ($_FILES['inp_image'] ['error']){
							case 1:
								$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$ll&ft=error&fm=to_big_file";
								header("Location: $url");
								exit;
								break;
							case 2:
								$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$l&ft=error&fm=to_big_file";
								header("Location: $url");
								exit;
								break;
							case 3:
								$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$l&ft=error&fm=only_parts_uploaded";
								header("Location: $url");
								exit;
								break;
							case 4:
								$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$l&ft=error&fm=no_file_uploaded";
								header("Location: $url");
								exit;
								break;
						}
					...
					$url = "new_exercise_step_9_images.php?action=upload_image&exercise_id=$exercise_id&l=$l&ft=error&fm=invalid_file_type&file_type=$file_type";
					header("Location: $url");
					exit;
				}
			...
			<form method=\"post\" action=\"new_exercise_step_9_images.php?action=upload_image&amp;exercise_id=$exercise_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
			
			<p><b>$l_new_image (890x890 png):</b><br />
			...
			<a href=\"new_exercise_step_9_images.php?exercise_id=$exercise_id&amp;l=$l&\" class=\"btn btn_default\">$l_images</a>
			<a href=\"new_exercise_step_10_video.php?exercise_id=$exercise_id&amp;l=$l&\" class=\"btn btn_default\">$l_continue</a>
			</p>
		...
		elseif($action == ""){
				...
				if($get_exercise_image_file != "" && file_exists("$root/$get_exercise_image_path/$get_exercise_image_file")){
					...
					echo"
					  <td style=\"padding-right: 10px;\">
						...
						<img src=\"$root/image.php?width=100&amp;height=67&amp;image=/$get_exercise_image_path/$get_exercise_image_file\" alt=\"$get_exercise_image_type\" />
						</p>
					  </td>";
				}
			}
			...
			<p>
			<a href=\"new_exercise_step_9_images.php?action=upload_image&amp;exercise_id=$exercise_id&amp;l=$l&\" class=\"btn\">$l_upload_image</a>
			<a href=\"new_exercise_step_10_video.php?exercise_id=$exercise_id&amp;l=$l&\" class=\"btn\">$l_continue</a>
			</p>
