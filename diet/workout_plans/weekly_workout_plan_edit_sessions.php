	...
	if($get_current_workout_weekly_id == ""){
				...
				while($row = mysqli_fetch_row($result)) {
					...
					<tr>
					  ...
					  <td class=\"$style\">
						...
						echo"
						<a href=\"weekly_workout_plan_edit_sessions.php?weekly_id=$weekly_id&amp;action=edit_session&amp;session_id=$get_workout_session_id&amp;l=$l\"><img src=\"_gfx/icons/edit.png\" alt=\"edit.png\" title=\"$l_edit\" /></a>
		...
		elseif($action == "new_session"){
			if($process == "1"){
				...
				$query_t = "SELECT workout_session_weight FROM $t_workout_plans_sessions WHERE workout_session_user_id=$my_user_id_mysql AND workout_session_weekly_id='$get_current_workout_weekly_id' ORDER BY workout_session_weight DESC";
			...
			<!-- Back -->
				...
				<a href=\"weekly_workout_plan_view.php?weekly_id=$weekly_id&amp;l=$l\" class=\"btn btn_default\">$l_next</a>
				</p>
			<!-- //Back -->
			";
		} // new
		...
		elseif($action == "add_exercise_to_session"){
			...
			if($get_workout_session_id == ""){
				...
				if($process == 1){
					...
					// Get correct weight
					$query = "SELECT * FROM $t_workout_plans_sessions_main WHERE workout_session_main_user_id=$my_user_id_mysql AND workout_session_main_session_id='$get_workout_session_id'";
					$result = mysqli_query($link, $query);
					...
					VALUES 
					(NULL, $my_user_id_mysql, '$get_workout_session_id', '$row_cnt', $inp_exercise_id_mysql, $inp_exercise_title_mysql, 
					...
					// Header
					$url = "weekly_workout_plan_edit_sessions.php?weekly_id=$weekly_id&action=$action&session_id=$session_id&l=$l&type_id=$type_id&main_muscle_group_id=$main_muscle_group_id&ft=success&fm=added";
					header("Location: $url");
					exit;
				}
				...
				<!-- Form -->
					...
					<form method=\"post\" action=\"weekly_workout_plan_edit_sessions.php?weekly_id=$weekly_id&amp;action=$action&amp;session_id=$session_id&amp;l=$l&amp;type_id=$type_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;process=1\" enctype=\"multipart/form-data\">
				...
				<a href=\"weekly_workout_plan_view.php?weekly_id=$weekly_id&amp;type_id=$type_id&amp;l=$l\" class=\"btn btn_default\">$l_next</a>
				</p>
				<!-- //Back -->
				";
			} //session found
