	...
	if($get_current_workout_weekly_id == ""){
		...
		if($action == ""){
				...
				while($row = mysqli_fetch_row($result)) {
					...
					<tr>
					  ...
					  <td class=\"$style\">
						...
						if($x != 0){
							echo"
							<a href=\"new_workout_plan_weekly_step_3_sessions.php?weekly_id=$weekly_id&amp;action=move_session_up&amp;session_id=$get_workout_session_id&amp;l=$l&amp;process=1\">$l_move_up</a>
							&middot;
							";
						}
							echo"
							<a href=\"new_workout_plan_weekly_step_3_sessions.php?weekly_id=$weekly_id&amp;action=move_session_down&amp;session_id=$get_workout_session_id&amp;l=$l&amp;process=1\">$l_move_down</a>
							&middot;
							";


						echo"
						<a href=\"new_workout_plan_weekly_step_3_sessions.php?weekly_id=$weekly_id&amp;action=edit_session&amp;session_id=$get_workout_session_id&amp;l=$l\">$l_edit</a>
						&middot;
						<a href=\"new_workout_plan_weekly_step_3_sessions.php?weekly_id=$weekly_id&amp;action=delete_session&amp;session_id=$get_workout_session_id&amp;l=$l\">$l_delete</a>
						</span>
					 </td>
					</tr>
					";
			...
			<!-- Back -->
				...
				<a href=\"weekly_workout_plan_edit_image.php?weekly_id=$weekly_id&amp;l=$l\" class=\"btn btn_default\">$l_next</a>
				</p>
			<!-- //Back -->
			";
		}
		elseif($action == "new_session"){
			...
			<!-- Form -->
				...
				<p><b>$l_goal:</b><br />
				<textarea name=\"inp_goal\" rows=\"10\" cols=\"70\" class=\"myTextEditor\" tabindex=\"";$tabindex=$tabindex+1;echo"$tabindex\"></textarea>
				</p>
		...
		elseif($action == "add_exercise_to_session"){
			...
			if($get_workout_session_id == ""){
				...
				if($process == 1){
					...
					// Header
					$url = "new_workout_plan_weekly_step_3_sessions.php?weekly_id=$weekly_id&action=$action&session_id=$session_id&l=$l&type_id=$type_id&main_muscle_group_id=$main_muscle_group_id&ft=success&fm=added";
					header("Location: $url");
					exit;
				}
				...
				<!-- Form -->
					...
					<form method=\"post\" action=\"new_workout_plan_weekly_step_3_sessions.php?weekly_id=$weekly_id&amp;action=$action&amp;session_id=$session_id&amp;l=$l&amp;type_id=$type_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;process=1\" enctype=\"multipart/form-data\">
					...
					<select name=\"inp_type_select\" id=\"inp_type_select\" tabindex=\"";$tabindex=$tabindex+1;echo"$tabindex\">
						...
						while($row_sub = mysqli_fetch_row($result_sub)) {
							...
							echo"		";
							echo"<option value=\"new_workout_plan_weekly_step_3_sessions.php?weekly_id=$weekly_id&amp;action=$action&amp;session_id=$session_id&amp;l=$l&amp;type_id=$get_type_id\""; if($type_id == "$get_type_id"){ echo" selected=\"selected\"";}echo">$get_type_translation_value</option>\n";
					...
					if($type_id != ""){
						...
						if($main_muscle_group_id != ""){
							...
							$x = 0;
							$query_exercises = "SELECT exercise_id, exercise_title FROM $t_exercise_index WHERE exercise_language=$l_mysql AND exercise_muscle_group_id_main=$main_muscle_group_id_mysql AND exercise_type_id=$type_id_mysql";
							...
							while($row_exercises = mysqli_fetch_row($result_exercises)) {
								...
								echo"		";
								echo"<option value=\"$get_exercise_id\">$get_exercise_title</option>\n";
							}
							echo"
							</select>
				...
				<a href=\"weekly_workout_plan_edit_image.php?weekly_id=$weekly_id&amp;type_id=$type_id&amp;l=$l\" class=\"btn btn_default\">$l_next</a>
				</p>
				<!-- //Back -->
				";
			} //session found
