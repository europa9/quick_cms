...
include("$root/_admin/_translations/site/$l/discuss/ts_view_topic.php");
...
if($get_current_topic_id == ""){
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$get_current_title_value - $get_current_topic_title";
	if(file_exists("./favicon.ico")){ $root = "."; }
	elseif(file_exists("../favicon.ico")){ $root = ".."; }
	elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
	elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
	...
	if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
		...
		if($process == "1"){
			...
			mysqli_query($link, "INSERT INTO $t_discuss_replies
			...
			// Get reply ID
			$query = "SELECT reply_id FROM $t_discuss_replies WHERE reply_user_id=$my_user_id_mysql AND reply_created='$datetime'";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			...
			$sql = "UPDATE $t_discuss_replies SET reply_text=? WHERE reply_id=$get_reply_id";
			...
			// E-mail subscribers
			$query_w = "SELECT * FROM $t_discuss_topics_subscribers WHERE topic_id='$get_current_topic_id'";
			...
			if($inp_notify_me_when_a_reply_is_posted == "on"){
				...
				$query = "SELECT topic_subscriber_id FROM $t_discuss_topics_subscribers WHERE topic_id='$get_current_topic_id' AND topic_subscriber_user_id=$my_user_id_mysql";
				$result = mysqli_query($link, $query);
				$row = mysqli_fetch_row($result);
			...
			$time = time();
			$result = mysqli_query($link, "UPDATE $t_discuss_topics SET topic_replies=$inp_topic_replies, topic_last_replied='$datetime', topic_last_replied_time='$time' WHERE topic_id=$topic_id_mysql");
			...
			$query = "SELECT top_monthly_id, top_monthly_user_id, top_monthly_year, top_monthly_month, top_monthly_topics, top_monthly_replies, top_monthly_times_voted, top_monthly_points FROM $t_discuss_top_users_monthly WHERE top_monthly_user_id=$my_user_id_mysql AND top_monthly_year=$year AND top_monthly_month=$month";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			...
			if($get_top_monthly_id == ""){
				...
				$result = mysqli_query($link, "UPDATE $t_discuss_top_users_monthly SET top_monthly_replies=$inp_top_monthly_replies, top_monthly_points=$inp_top_monthly_points, top_monthly_user_alias=$inp_my_user_alias_mysql, top_monthly_user_image=$inp_reply_user_image_mysql WHERE top_monthly_id='$get_top_monthly_id'");
			...
			if($get_top_yearly_id == ""){
				...
				$result = mysqli_query($link, "UPDATE $t_discuss_top_users_yearly SET top_yearly_replies=$inp_top_yearly_replies, top_yearly_points=$inp_top_yearly_points, top_yearly_user_alias=$inp_my_user_alias_mysql, top_yearly_user_image=$inp_reply_user_image_mysql WHERE top_yearly_id='$get_top_yearly_id'") or die(mysqli_error($link));
			}
			...
			if($get_top_all_id == ""){
				...
				$result = mysqli_query($link, "UPDATE $t_discuss_top_users_all_time SET top_all_replies=$inp_top_all_replies, top_all_points=$inp_top_all_points, top_all_user_alias=$inp_my_user_alias_mysql, top_all_user_image=$inp_reply_user_image_mysql WHERE top_all_id='$get_top_all_id'");
			}
			...
			$datetime = date("Y-m-d H:i:s");
			$time = time();
			$result = mysqli_query($link, "UPDATE $t_discuss_topics SET topic_updated='$datetime', topic_updated_time='$time' WHERE topic_id='$get_current_topic_id'") or die(mysqli_error($link));
