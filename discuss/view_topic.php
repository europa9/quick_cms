...
if($get_current_topic_id == ""){
	...
	if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
		...
		// Mark it read
		$query = "SELECT topic_read_id FROM $t_discuss_topics_read_by_user WHERE topic_read_topic_id=$get_current_topic_id AND topic_read_user_id=$my_user_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		list($get_topic_read_id) = $row;
		if($get_topic_read_id == ""){
			mysqli_query($link, "INSERT INTO $t_discuss_topics_read_by_user
		...
		// By IP
		$query = "SELECT topic_read_id FROM $t_discuss_topics_read_by_ip WHERE topic_read_topic_id=$get_current_topic_id AND topic_read_ip=$my_ip_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		list($get_topic_read_id) = $row;
		if($get_topic_read_id == ""){
			$year = date("Y");
			mysqli_query($link, "INSERT INTO $t_discuss_topics_read_by_ip
			...
			$result = mysqli_query($link, "DELETE FROM $t_discuss_topics_read_by_ip WHERE topic_read_year=$last_year") or die(mysqli_error($link));
	...
	if($has_seen_this_before == 0){
		...
		$result = mysqli_query($link, "UPDATE $t_discuss_topics SET topic_views=$inp_unique_hits, topic_views_ip_block=$ip_block_mysql WHERE topic_id=$get_current_topic_id") or die(mysqli_error($link));
	}
	...
	<!-- Replies -->
		...
		while($row_w = mysqli_fetch_row($result_w)) {
			...
			if($get_current_topic_solved == "0" && $days_since_written > 14){
				$result_update = mysqli_query($link, "UPDATE $t_discuss_topics SET topic_solved='1' WHERE topic_id=$get_current_topic_id") or die(mysqli_error($link));
