		...
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			if(isset($can_edit) && $can_edit == "true"){
				if($process == "1"){
					...
					$result = mysqli_query($link, "DELETE FROM $t_discuss_replies WHERE reply_id=$reply_id_mysql");

					// Update reply counter
					$inp_topic_replies = $get_current_topic_replies - 1;
					$result = mysqli_query($link, "UPDATE $t_discuss_topics SET topic_replies=$inp_topic_replies WHERE topic_id=$topic_id_mysql");
