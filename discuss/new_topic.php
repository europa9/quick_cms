...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$get_current_title_value - $l_new_topic";
if(file_exists("./favicon.ico")){ $root = "."; }
elseif(file_exists("../favicon.ico")){ $root = ".."; }
elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($process == "1"){
		...
		mysqli_query($link, "INSERT INTO $t_discuss_topics
		...
		// Get ID
		$query = "SELECT topic_id FROM $t_discuss_topics WHERE topic_user_id=$my_user_id_mysql AND topic_created='$datetime'";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		$sql = "UPDATE $t_discuss_topics SET topic_text=? WHERE topic_id=$get_topic_id";
		...
		if(isset($_POST['inp_notify_me_when_a_reply_is_posted'])){
			...
			if($inp_notify_me_when_a_reply_is_posted == "on"){
				...
				mysqli_query($link, "INSERT INTO $t_discuss_topics_subscribers
		...
		if($size > 0){
			for($x=0;$x<$size;$x++){
				...
				$query = "SELECT topic_tag_id FROM $t_discuss_topics_tags WHERE topic_id=$get_topic_id AND topic_tag_clean=$inp_tag_clean_mysql";
				$result = mysqli_query($link, $query);
				$row = mysqli_fetch_row($result);
				list($get_topic_tag_id) = $row;
				if($get_topic_tag_id == ""){
					// Insert
					mysqli_query($link, "INSERT INTO $t_discuss_topics_tags 
				...
				$query = "SELECT tag_id, tag_topics_total_counter, tag_topics_today_counter, tag_topics_today_day, tag_topics_this_week_counter, tag_topics_this_week_week FROM $t_discuss_tags_index WHERE tag_title_clean=$inp_tag_clean_mysql";
				$result = mysqli_query($link, $query);
				$row = mysqli_fetch_row($result);
				...
				if($get_tag_id == ""){
					// Insert
					mysqli_query($link, "INSERT INTO $t_discuss_tags_index 
		...
		$query_w = "SELECT discuss_subscription_id, discuss_subscription_user_id, discuss_subscription_user_email, discuss_subscription_last_sendt_datetime, discuss_subscription_last_sendt_time FROM $t_discuss_subscriptions";
...
div.topics_chat_view_text {
	...
	margin-bottom: 10px;
		...
		$query = "SELECT top_monthly_id, top_monthly_user_id, top_monthly_topics, top_monthly_replies, top_monthly_times_voted, top_monthly_year, top_monthly_month, top_monthly_points FROM $t_discuss_top_users_monthly WHERE top_monthly_user_id=$my_user_id_mysql AND top_monthly_year=$year AND top_monthly_month=$month";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if($get_top_monthly_id == ""){
			...
			mysqli_query($link, "INSERT INTO $t_discuss_top_users_monthly 
			...
			$result = mysqli_query($link, "UPDATE $t_discuss_top_users_monthly SET top_monthly_topics=$inp_top_monthly_topics, top_monthly_points=$inp_top_monthly_points, top_monthly_user_alias=$inp_my_user_alias_mysql, top_monthly_user_image=$inp_topic_user_image_mysql WHERE top_monthly_id='$get_top_monthly_id'");

		}

		// Top users yearly
		$query = "SELECT top_yearly_id, top_yearly_user_id, top_yearly_topics, top_yearly_replies, top_yearly_times_voted, top_yearly_year, top_yearly_points FROM $t_discuss_top_users_yearly WHERE top_yearly_user_id=$my_user_id_mysql AND top_yearly_year=$year";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if($get_top_yearly_id == ""){
			// First time I posted this month
			mysqli_query($link, "INSERT INTO $t_discuss_top_users_yearly 
			...
			$result = mysqli_query($link, "UPDATE $t_discuss_top_users_yearly SET top_yearly_topics=$inp_top_yearly_topics, top_yearly_points=$inp_top_yearly_points, top_yearly_user_alias=$inp_my_user_alias_mysql, top_yearly_user_image=$inp_topic_user_image_mysql WHERE top_yearly_id='$get_top_yearly_id'") or die(mysqli_error($link));
		}


		// Top users all time
		$query = "SELECT top_all_id, top_all_user_id, top_all_topics, top_all_replies, top_all_times_voted, top_all_points FROM $t_discuss_top_users_all_time WHERE top_all_user_id=$my_user_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if($get_top_all_id == ""){
			...
			mysqli_query($link, "INSERT INTO $t_discuss_top_users_all_time
			...
			$result = mysqli_query($link, "UPDATE $t_discuss_top_users_all_time SET top_all_topics=$inp_top_all_topics, top_all_points=$inp_top_all_points, top_all_user_alias=$inp_my_user_alias_mysql, top_all_user_image=$inp_topic_user_image_mysql WHERE top_all_id='$get_top_all_id'");
