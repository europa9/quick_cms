...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$get_current_title_value - $l_watched_tags";
if(file_exists("./favicon.ico")){ $root = "."; }
elseif(file_exists("../favicon.ico")){ $root = ".."; }
elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	elseif($action == "unwatch"){
		...
		$query = "SELECT watch_id, watch_tag_id, watch_user_id, watch_user_name, watch_user_email, watch_user_email_notification, watch_user_last_sent_email_datetime, watch_user_last_sent_email_time FROM $t_discuss_tags_watch WHERE watch_id=$watch_id_mysql AND watch_user_id=$my_user_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if($get_current_watch_id == ""){
			...
			// Get tag
			$query = "SELECT tag_id, tag_title, tag_title_clean FROM $t_discuss_tags_index WHERE tag_id=$get_current_watch_tag_id";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			list($get_tag_id, $get_tag_title, $get_tag_title_clean) = $row;
		
			if($process == "1"){
				$result = mysqli_query($link, "DELETE FROM $t_discuss_tags_watch WHERE watch_id=$watch_id_mysql AND watch_user_id=$my_user_id_mysql");
