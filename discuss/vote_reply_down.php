...
/**
*
* File: discuss/vote_reply_down.php
* Version 1.0.0
...
if($topic_id == "" && $reply_id != ""){
	...
	$query = "SELECT reply_id, reply_topic_id FROM $t_discuss_replies WHERE reply_id=$reply_id_mysql";
...
if($get_current_topic_id == ""){
	...
	// Get reply
	$reply_id_mysql = quote_smart($link, $reply_id);
	$query = "SELECT reply_id, reply_user_id, reply_user_alias, reply_user_image, reply_topic_id, reply_text, reply_created, reply_updated, reply_updated_translated, reply_selected_answer, reply_likes, reply_dislikes, reply_rating, reply_likes_ip_block, reply_user_ip FROM $t_discuss_replies WHERE reply_id=$reply_id_mysql";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	...
	if($get_current_reply_id == ""){
		...
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			$result = mysqli_query($link, "UPDATE $t_discuss_replies SET 
			...
			if($get_top_monthly_id == ""){
				...
				$result = mysqli_query($link, "UPDATE $t_discuss_top_users_monthly SET top_monthly_times_voted=$inp_top_monthly_times_voted, top_monthly_points=$inp_top_monthly_points, top_monthly_user_alias=$inp_my_user_alias_mysql, top_monthly_user_image=$inp_my_photo_destination_mysql WHERE top_monthly_id='$get_top_monthly_id'");
			...
			if($get_top_yearly_id == ""){
				...
				$result = mysqli_query($link, "UPDATE $t_discuss_top_users_yearly SET top_yearly_times_voted=$inp_top_yearly_times_voted, top_yearly_points=$inp_top_yearly_points, top_yearly_user_alias=$inp_my_user_alias_mysql, top_yearly_user_image=$inp_my_photo_destination_mysql WHERE top_yearly_id='$get_top_yearly_id'") or die(mysqli_error($link));
			}
			...
			if($get_top_all_id == ""){
				...
				$result = mysqli_query($link, "UPDATE $t_discuss_top_users_all_time SET top_all_times_voted=$inp_top_all_times_voted, top_all_points=$inp_top_all_points, top_all_user_alias=$inp_my_user_alias_mysql, top_all_user_image=$inp_my_photo_destination_mysql WHERE top_all_id='$get_top_all_id'");
