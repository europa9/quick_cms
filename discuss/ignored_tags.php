...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$get_current_title_value - $l_ignored_tags";
if(file_exists("./favicon.ico")){ $root = "."; }
elseif(file_exists("../favicon.ico")){ $root = ".."; }
elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($action == ""){
		...
		<!-- Watched tags -->
			<div class=\"vertical_list\">
				<ul>
				";
				$query_w = "SELECT ignore_id, ignore_tag_id, ignore_user_id, tag_id, tag_title_clean, tag_icon_path, tag_icon_file_16  FROM $t_discuss_tags_ignore JOIN $t_discuss_tags_index ON $t_discuss_tags_ignore.ignore_tag_id=$t_discuss_tags_index.tag_id WHERE $t_discuss_tags_ignore.ignore_user_id=$my_user_id_mysql";
	...
	elseif($action == "remove_from_ignore_list"){
		...
		$query = "SELECT ignore_id, ignore_tag_id, ignore_user_id FROM $t_discuss_tags_ignore WHERE ignore_id=$ignore_id_mysql AND ignore_user_id=$my_user_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if($get_current_ignore_id == ""){
			...
			// Get tag
			$query = "SELECT tag_id, tag_title, tag_title_clean FROM $t_discuss_tags_index WHERE tag_id=$get_current_ignore_tag_id";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			list($get_tag_id, $get_tag_title, $get_tag_title_clean) = $row;
		
			if($process == "1"){
				$result = mysqli_query($link, "DELETE FROM $t_discuss_tags_ignore WHERE ignore_id=$ignore_id_mysql AND ignore_user_id=$my_user_id_mysql");
