...
<!-- Hall of fame -->
	...
	<div class=\"hall_of_fame_wrapper\">
		...
		$year = date("Y");
		$month = date("m");
		$query_w = "SELECT top_yearly_id, top_yearly_user_id, top_yearly_year, top_yearly_topics, top_yearly_replies, top_yearly_times_voted, top_yearly_points, top_yearly_user_alias, top_yearly_user_image FROM $t_discuss_top_users_yearly WHERE top_yearly_year=$year ORDER BY top_yearly_points DESC LIMIT 0,8";
...
if($action == ""){
	...
	$x = 0;
	$query_w = "SELECT topic_id, topic_user_id, topic_user_alias, topic_user_image, topic_title, topic_updated_translated, topic_replies, topic_views, topic_solved FROM $t_discuss_topics WHERE topic_language=$l_mysql ORDER BY topic_last_replied DESC";
	$result_w = mysqli_query($link, $query_w);
	while($row_w = mysqli_fetch_row($result_w)) {
...
elseif($action == "unanswered"){
	...
	$query_w = "SELECT topic_id, topic_user_id, topic_user_alias, topic_user_image, topic_title, topic_updated_translated, topic_replies, topic_views, topic_solved FROM $t_discuss_topics WHERE topic_language=$l_mysql AND topic_replies='0' ORDER BY topic_last_replied DESC";
	$result_w = mysqli_query($link, $query_w);
	while($row_w = mysqli_fetch_row($result_w)) {
		list($get_topic_id, $get_topic_user_id, $get_topic_user_alias, $get_topic_user_image, $get_topic_title, $get_topic_updated_translated, $get_topic_replies, $get_topic_views, $get_topic_solved) = $row_w;
