...
if($get_current_topic_id == ""){
	...
	if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
		...
		$query = "SELECT topic_subscriber_id FROM $t_discuss_topics_subscribers WHERE topic_id=$get_current_topic_id AND topic_subscriber_user_id=$get_current_topic_user_id";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if(isset($can_edit) && $can_edit == "true"){
			if($process == "1"){
				...
				$sql = "UPDATE $t_discuss_topics SET topic_text=? WHERE topic_id=$get_current_topic_id";
				...
				$r_delete = mysqli_query($link, "DELETE FROM $t_discuss_tags_index $t_discuss_topics_tags WHERE topic_id=$get_current_topic_id");
				...
				if($size > 0){
					for($x=0;$x<$size;$x++){
						...
						$query = "SELECT topic_tag_id FROM $t_discuss_topics_tags WHERE topic_id=$get_current_topic_id AND topic_tag_clean=$inp_tag_clean_mysql";
						$result = mysqli_query($link, $query);
						$row = mysqli_fetch_row($result);
		...
		tinymce.init({
			...
			<!-- Form -->
				...
				$query_t = "SELECT topic_tag_id, topic_tag_title, topic_tag_clean FROM $t_discuss_topics_tags WHERE topic_id=$get_current_topic_id";
				$result_t = mysqli_query($link, $query_t);
