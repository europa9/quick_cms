...
/**
*
* File: discuss/reply_comment_delete.php
* Version 1.0.0
...
include("$root/_admin/_translations/site/$l/discuss/ts_reply_add_a_comment.php");
include("$root/_admin/_translations/site/$l/discuss/ts_reply_comment_edit.php");
...
if($get_current_reply_comment_id == ""){
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$l_discuss - $get_current_topic_title - $l_delete_comment";
	if(file_exists("./favicon.ico")){ $root = "."; }
	elseif(file_exists("../favicon.ico")){ $root = ".."; }
	elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
	elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
	...
	if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
		...
		if($get_my_user_id == "$get_current_reply_comment_user_id"){
			...
			if($process == "1"){
				...
				// Delete
				$result = mysqli_query($link, "DELETE FROM $t_discuss_replies_comments WHERE reply_comment_id=$get_current_reply_comment_id") or die(mysqli_error($link));
