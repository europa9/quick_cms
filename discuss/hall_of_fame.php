...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$get_current_title_value - $l_hall_of_fame";
if(file_exists("./favicon.ico")){ $root = "."; }
elseif(file_exists("../favicon.ico")){ $root = ".."; }
elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
...
if($action == ""){
	...
	<!-- Hall of fame -->
		...
		$month = date("m");
		$x = 1;
		$query_w = "SELECT top_all_id, top_all_user_id, top_all_topics, top_all_replies, top_all_times_voted, top_all_points, top_all_user_alias, top_all_user_image FROM $t_discuss_top_users_all_time ORDER BY top_all_points DESC LIMIT 0,100";
...
elseif($action == "monthly"){
	...
	echo"</h2>
		...
		$x = 1;
		$query_w = "SELECT top_monthly_id, top_monthly_user_id, top_monthly_year, top_monthly_month, top_monthly_topics, top_monthly_replies, top_monthly_times_voted, top_monthly_points, top_monthly_user_alias, top_monthly_user_image FROM $t_discuss_top_users_monthly WHERE top_monthly_year=$year AND top_monthly_month=$month ORDER BY top_monthly_points DESC LIMIT 0,8";
		$result_w = mysqli_query($link, $query_w);
		while($row_w = mysqli_fetch_row($result_w)) {
