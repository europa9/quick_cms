...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$get_current_title_value - $l_tags";
if(file_exists("./favicon.ico")){ $root = "."; }
elseif(file_exists("../favicon.ico")){ $root = ".."; }
elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
...
if($action == ""){
	...
	<!-- Tags -->
		<div class=\"forum_grids\">
			...
			";
			$l_mysql = quote_smart($link, $l);
			$query_w = "SELECT tag_id, tag_title, tag_title_clean, tag_introduction, tag_description, tag_topics_total_counter, tag_topics_today_counter, tag_topics_this_week_counter, tag_icon_path, tag_icon_file_16 FROM $t_discuss_tags_index WHERE tag_is_official='1' ORDER BY tag_topics_total_counter DESC";
			...
			while($row_w = mysqli_fetch_row($result_w)) {
				...
				// Translation
				$query_tag = "SELECT tag_translation_id, tag_id, tag_translation_language, tag_translation_introduction, tag_translation_description FROM $t_discuss_tags_index_translation WHERE tag_id=$get_tag_id AND tag_translation_language=$l_mysql";
