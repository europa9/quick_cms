...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$get_current_title_value - $l_newest $tag $l_topics_lowercase";
if(file_exists("./favicon.ico")){ $root = "."; }
elseif(file_exists("../favicon.ico")){ $root = ".."; }
elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
...
if($get_current_tag_id == ""){
	...
	// Translation
	$l_mysql = quote_smart($link, $l);
	$query_tag = "SELECT tag_translation_id, tag_id, tag_translation_language, tag_translation_introduction, tag_translation_description FROM $t_discuss_tags_index_translation WHERE tag_id=$get_current_tag_id AND tag_translation_language=$l_mysql";
	$result_tag = mysqli_query($link, $query_tag);
	$row_tag = mysqli_fetch_row($result_tag);
	...
	if($get_current_tag_translation_id == ""){
		...
		mysqli_query($link, "INSERT INTO $t_discuss_tags_index_translation 
	...
	if($action == ""){
		...
		<!-- Menu -->
			...
			<div style=\"float: right;\">
				...
				if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
					...
					$query_tag = "SELECT watch_id, watch_tag_id, watch_user_id, watch_user_name, watch_user_email, watch_user_email_notification, watch_user_last_sent_email_datetime, watch_user_last_sent_email_time FROM $t_discuss_tags_watch WHERE watch_tag_id=$get_current_tag_id AND watch_user_id=$my_user_id_mysql";
					$result_tag = mysqli_query($link, $query_tag);
					$row_tag = mysqli_fetch_row($result_tag);
					...
					$query_tag = "SELECT ignore_id, ignore_tag_id, ignore_user_id FROM $t_discuss_tags_ignore WHERE ignore_tag_id=$get_current_tag_id AND ignore_user_id=$my_user_id_mysql";
					$result_tag = mysqli_query($link, $query_tag);
					$row_tag = mysqli_fetch_row($result_tag);
		...
		$query_w = "SELECT $t_discuss_topics_tags.topic_id, $t_discuss_topics.topic_user_id, $t_discuss_topics.topic_user_alias, $t_discuss_topics.topic_user_image, $t_discuss_topics.topic_title, $t_discuss_topics.topic_updated_translated, $t_discuss_topics.topic_replies, $t_discuss_topics.topic_views, $t_discuss_topics.topic_solved FROM $t_discuss_topics_tags JOIN $t_discuss_topics ON $t_discuss_topics_tags.topic_id=$t_discuss_topics.topic_id WHERE $t_discuss_topics_tags.topic_tag_clean=$tag_title_clean_mysql ORDER BY $t_discuss_topics_tags.topic_id DESC";
		...
		if(isset($get_my_user_id)){
			$query = "SELECT topic_read_id FROM $t_discuss_topics_read_by_user WHERE topic_read_topic_id=$get_topic_id AND topic_read_user_id=$my_user_id_mysql";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			list($get_topic_read_id) = $row;
		}
		else{
			$query = "SELECT topic_read_id FROM $t_discuss_topics_read_by_ip WHERE topic_read_topic_id=$get_topic_id AND topic_read_ip=$my_ip_mysql";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			list($get_topic_read_id) = $row;
		}
			...
			<table style=\"width: 100%;\">
			 <tr>
			  <td style=\"vertical-align: center;\">
				...
				$query_t = "SELECT topic_tag_id, topic_tag_title, topic_tag_clean FROM $t_discuss_topics_tags WHERE topic_id=$get_topic_id";
				$result_t = mysqli_query($link, $query_t);
				while($row_t = mysqli_fetch_row($result_t)) {
					...
					$query_tag = "SELECT tag_id, tag_icon_path, tag_icon_file_16 FROM $t_discuss_tags_index WHERE tag_title_clean=$tag_title_clean_mysql";
					$result_tag = mysqli_query($link, $query_tag);
					$row_tag = mysqli_fetch_row($result_tag);
	...
	elseif($action == "watch_tag"){
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			if($get_watch_id == ""){
				// Insert to watch
				mysqli_query($link, "INSERT INTO $t_discuss_tags_watch 
				...
				// Update watch
				$result = mysqli_query($link, "UPDATE $t_discuss_tags_watch SET watch_user_name=$inp_user_name_mysql, watch_user_email=$inp_user_email_mysql, watch_user_email_notification='0' WHERE watch_id=$get_watch_id");
			}

			// Delete from ignore list (if it is there)
			$result = mysqli_query($link, "DELETE FROM $t_discuss_tags_ignore WHERE ignore_tag_id=$get_current_tag_id AND ignore_user_id=$get_my_user_id");
	...
	elseif($action == "watch_and_subscribe_tag"){
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			if($get_watch_id == ""){
				...
				// Update watch
				$result = mysqli_query($link, "UPDATE $t_discuss_tags_watch SET watch_user_name=$inp_user_name_mysql, watch_user_email=$inp_user_email_mysql, watch_user_email_notification='1' WHERE watch_id=$get_watch_id");
			}
	...
	elseif($action == "unwatch_tag"){
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			if($get_watch_id != ""){
				// Update watch
				$result = mysqli_query($link, "DELETE FROM $t_discuss_tags_watch WHERE watch_tag_id=$get_current_tag_id AND watch_user_id=$my_user_id_mysql");
			}
	...
	elseif($action == "ignore_tag"){
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			// Am I already watching it?
			$query_tag = "SELECT ignore_id FROM $t_discuss_tags_ignore WHERE ignore_tag_id=$get_current_tag_id AND ignore_user_id=$get_my_user_id";
			$result_tag = mysqli_query($link, $query_tag);
			$row_tag = mysqli_fetch_row($result_tag);
			list($get_ignore_id) = $row_tag;
			if($get_ignore_id == ""){
				// Insert to ignore
				mysqli_query($link, "INSERT INTO $t_discuss_tags_ignore 
