	...
	if($get_current_reply_id == ""){
		...
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			if($process == "1"){
				...
				// Insert comment
				mysqli_query($link, "INSERT INTO $t_discuss_replies_comments
				...
				// Get ID
				$query = "SELECT reply_comment_id FROM $t_discuss_replies_comments WHERE reply_comment_user_id=$my_user_id_mysql AND reply_comment_created='$datetime'";
				$result = mysqli_query($link, $query);
				$row = mysqli_fetch_row($result);
