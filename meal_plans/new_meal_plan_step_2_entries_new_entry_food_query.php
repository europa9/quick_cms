...
/**
*
* File: meal_plans/new_meal_plan_step_2_entries_new_entry_food_query.php
* Version 1.0.0
...
$t_diet_food_queries 	= $mysqlPrefixSav . "diet_food_queries";
...
if(isset($_GET['entry_meal_number']) OR isset($_POST['entry_meal_number'])) {
	...
	echo"Missing entry_meal_number";
	die;
}
...
/*- Table exists? -------------------------------------------------------------------- */
$query = "SELECT * FROM $t_diet_food_queries LIMIT 1";
$result = mysqli_query($link, $query);
if($result !== FALSE){
}
else{
	echo"Table created";
	mysqli_query($link, "CREATE TABLE $t_diet_food_queries(
...
if(isset($_GET['q']) OR isset($_POST['q'])){
	...
	if($q != ""){
		$query = "SELECT query_name, query_times FROM $t_diet_food_queries WHERE query_name=$q_mysql";
		...
		if($get_query_name == ""){
			// Insert
			$insert_error = "0";
			mysqli_query($link, "INSERT INTO $t_diet_food_queries
			...
			$result = mysqli_query($link, "UPDATE $t_diet_food_queries SET query_times='$inp_query_times', query_last_use='$inp_datetime' WHERE query_name=$q_mysql");
		}
