...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($process == "1"){
		...
		if($get_group_id != ""){
			$url = "new_group.php?l=$l&ft=error&fm=there_is_already_a_group_with_that_name_(" . $inp_name . ")";
			header("Location: $url");
			exit;
		}
		...
		(member_id, member_group_id, member_status, member_accepted_by_moderator, member_user_id, 
		member_user_name, member_user_email, member_user_photo_destination, member_user_photo_thumb_50, member_joined_datetime, 
		member_joined_date_saying) 
		VALUES 
		(NULL, $get_current_group_id, 'admin', 1, $get_my_user_id, 
		$inp_my_user_name_mysql, $inp_my_user_email_mysql, $inp_my_photo_destination_mysql, $inp_my_photo_thumb_50_mysql, '$datetime', 
		'$date_saying')")
		or die(mysqli_error($link));

		// Open group
		$url = "open_group.php?group_id=$get_current_group_id&l=$l&ft=success&fm=group_created";
		header("Location: $url");
		exit;
	...
	<!-- Where am I ? -->
		...
		&gt;
		<a href=\"new_group.php?l=$l\">$l_new_group</a>
		</p>
	<!-- //Where am I ? -->
	...
	<!-- New group form -->
		<form method=\"post\" action=\"new_group.php?l=$l&amp;process=1\" enctype=\"multipart/form-data\">
	...
	Loading...</h1>
	<meta http-equiv=\"refresh\" content=\"1;url=$root/users/login.php?l=$l&amp;referer=$root/rebus/new_group.php\">
