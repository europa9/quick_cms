...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($action == ""){
		...
		<p><a href=\"$root/talk/new_channel.php?l=$l\" class=\"btn_default\">$l_new_channel</a></p>
		...
		<!-- All channels -->
			...
			<div class=\"vertical\" style=\"width: 100%;\">
				...
				";
				$time = time();
				$query = "SELECT channel_id, channel_name, channel_password, channel_last_message_time, channel_last_message_saying, channel_users_online FROM $t_talk_channels_index ORDER BY channel_name ASC";
				$result = mysqli_query($link, $query);
				while($row = mysqli_fetch_row($result)) {
					...
					if($days_since_last_message > 365){
						...
						$result_del = mysqli_query($link, "DELETE FROM $t_talk_channels_index WHERE channel_id=$get_channel_id");
						$result_del = mysqli_query($link, "DELETE FROM $t_talk_channels_messages WHERE message_channel_id=$get_channel_id");
	...
	elseif($action == "join_without_password"){
		...
		// Find channel
		$query = "SELECT channel_id, channel_name, channel_password, channel_last_message_time FROM $t_talk_channels_index WHERE channel_id=$channel_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if($get_current_channel_id == ""){
			...
			// Already starred?
			$query = "SELECT starred_channel_id FROM $t_talk_users_starred_channels WHERE channel_id=$get_current_channel_id AND user_id=$get_my_user_id";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
