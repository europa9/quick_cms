...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($get_current_starred_channel_id == ""){
		...
		if($get_current_channel_id == ""){
			...
			// Make myself online
			$query = "SELECT online_id, online_time, online_is_online, online_user_id, online_user_name, online_user_alias FROM $t_talk_channels_users_online WHERE online_channel_id=$get_current_channel_id AND online_user_id=$my_user_id_mysql";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			...
			if($get_online_id == ""){
				...
				if($get_online_is_online == "0"){
					...
					$result_update_online = mysqli_query($link, "UPDATE $t_talk_channels_users_online SET online_time='$time', online_is_online=1 WHERE online_id=$get_online_id") or die(mysqli_error($link));
				}
			...
			if($get_current_channel_users_online != "$count_users_online"){
				$result = mysqli_query($link, "UPDATE $t_talk_channels_index SET channel_users_online=$count_users_online WHERE channel_id=$get_current_channel_id");
