...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	elseif($action == "delete"){
		...
		$query = "SELECT conversation_id, conversation_key, conversation_f_user_id, conversation_f_user_name, conversation_f_user_alias, conversation_f_image_path, conversation_f_image_file, conversation_f_image_thumb40, conversation_f_image_thumb50, conversation_f_has_blocked, conversation_f_unread_messages, conversation_t_user_id, conversation_t_user_name, conversation_t_user_alias, conversation_t_image_path, conversation_t_image_file, conversation_t_image_thumb40, conversation_t_image_thumb50, conversation_t_has_blocked, conversation_t_unread_messages FROM $t_talk_dm_conversations WHERE conversation_id=$conversation_id_mysql AND conversation_f_user_id=$my_user_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		...
		if($get_current_conversation_id == ""){
			...
			if($process == "1"){
				$result = mysqli_query($link, "DELETE FROM $t_talk_dm_conversations WHERE conversation_id=$get_current_conversation_id") or die(mysqli_error($link));
	...
	elseif($action == "block"){
		...
		if($get_current_conversation_id == ""){
			...
			if($process == "1"){
				$result = mysqli_query($link, "UPDATE $t_talk_dm_conversations SET conversation_f_has_blocked=1 WHERE conversation_id=$get_current_conversation_id") or die(mysqli_error($link));
	...
	elseif($action == "unblock"){
		...
		if($get_current_conversation_id == ""){
			...
			if($process == "1"){
				$result = mysqli_query($link, "UPDATE $t_talk_dm_conversations SET conversation_f_has_blocked=0 WHERE conversation_id=$get_current_conversation_id") or die(mysqli_error($link));
