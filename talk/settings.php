...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($action == ""){
		if($process == "1"){
			...
			// CHeck if taken
			$query = "SELECT nickname_id, nickname_user_id FROM $t_talk_nicknames WHERE nickname_value=$inp_nickname_value_mysql";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			...
			if($get_check_nickname_id == ""){
				...
				// Update my nickname
				$result = mysqli_query($link, "UPDATE $t_talk_nicknames SET nickname_value=$inp_nickname_value_mysql, nickname_datetime='$datetime', nickname_datetime_saying='$datetime_saying' WHERE nickname_id=$get_my_nickname_id") or die(mysqli_error($link));


				// Insert history
				$inp_change_from_value_mysql = quote_smart($link, $get_my_nickname_value);
				mysqli_query($link, "INSERT INTO $t_talk_nicknames_changes 
				...
				while($row = mysqli_fetch_row($result)) {
					...
					$result_update = mysqli_query($link, "UPDATE $t_talk_channels_users_online SET online_user_nickname=$inp_nickname_value_mysql WHERE online_user_id=$get_my_user_id") or die(mysqli_error($link));
				...
				$result_update = mysqli_query($link, "UPDATE $t_talk_dm_conversations SET conversation_t_user_nickname=$inp_nickname_value_mysql WHERE conversation_t_user_id=$get_my_user_id") or die(mysqli_error($link));
				$result_update = mysqli_query($link, "UPDATE $t_talk_dm_conversations SET conversation_f_user_nickname=$inp_nickname_value_mysql WHERE conversation_f_user_id=$get_my_user_id") or die(mysqli_error($link));
