...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	// Channel messages
	$query = "SELECT SUM(new_messages) FROM $t_talk_users_starred_channels WHERE user_id=$my_user_id_mysql";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	list($count_new_messages) = $row;

	// Conversation messages
	$query = "SELECT SUM(conversation_f_unread_messages) FROM $t_talk_dm_conversations WHERE conversation_f_user_id=$my_user_id_mysql AND conversation_f_has_blocked=0";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	...
	if($get_total_unread_id == ""){
		mysqli_query($link, "INSERT INTO $t_talk_total_unread 
		...
		if($get_total_unread_count != "$total_messages"){
			$result = mysqli_query($link, "UPDATE $t_talk_total_unread SET total_unread_count=$total_messages WHERE total_unread_user_id=$my_user_id_mysql");
