...
if($action == ""){
	...
	";
	$x = 0;
	$query = "SELECT restriction_id, restriction_country_name, restriction_country_iso_two, restriction_country_flag_path_16x16, restriction_country_flag_16x16, restriction_language, restriction_age_limit, restriction_title, restriction_text, restriction_can_view_food, restriction_can_view_images FROM $t_food_age_restrictions ORDER BY restriction_country_name ASC";
	$result = mysqli_query($link, $query);
	while($row = mysqli_fetch_row($result)) {
		list($get_restriction_id, $get_restriction_country_name, $get_restriction_country_iso_two, $get_restriction_country_flag_path_16x16, $get_restriction_country_flag_16x16, $get_restriction_language, $get_restriction_age_limit, $get_restriction_title, $get_restriction_text, $get_restriction_can_view_food, $get_restriction_can_view_images) = $row;
...
elseif($action == "view_agreement"){
	...
	if($get_current_restriction_id == ""){
		...
		echo"<h1>$get_current_restriction_title</h1>
